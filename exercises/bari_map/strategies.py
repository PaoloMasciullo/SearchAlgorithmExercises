...
class UniformCost:
    ...
    def select(self, fringe, new_nodes):
        ...
        # sort fringe in descending order of path cost
        fringe = sorted(fringe, key=lambda n: n.cost, reverse=True)
        return fringe
...
class Astar:
    ...
    def select(self, fringe, new_nodes):
        fringe = fringe + new_nodes
        fringe = [n for n in fringe if n.state not in self.visited]
        # sort fringe in descending order following the heuristic function and the path cost
        fringe = sorted(fringe, key=lambda n: (n.cost + self.problem.h(n.state)), reverse=True)
        return fringe
