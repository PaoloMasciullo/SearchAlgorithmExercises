...
def degree_heuristic(problem, state):
    """
    Given a state returns an unassigned variable to assign that are involved in the highest number of constrains
    @param problem:
    @param state:
    @return:
    """
...
def minimum_remaining_values(problem, state):
    """
    Given a state returns an unassigned variable to assign that have the fewest number of legal values in its domain
    @param problem:
    @param state:
    @return:
    """
...
class BackTracking:
    ...
    def run_with_forward_checking(self, state, domains):
        ...
        for value in values:
            ...
            if self.problem.consistent(new_state):
                ...
                # run the forward checking
                new_domains = self.forward_checking(state, domains)  # compute the new domains
                del (new_domains[variable])  # delete the variable just assigned

                # run the search on the new state and the new domains
                result = self.run_with_forward_checking(state=dict(state), domains=new_domains)
    ...
    def run_with_ac3(self, state):
        ...
        # check if there is any empty domain due to the zc3
        ...
        for value in values:
            ...
            if self.problem.consistent(new_state):
                ...
                # after the assignment I impose the acr consistency on all the graph
